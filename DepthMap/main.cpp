...
CvPoint VertexOne;
Mat color_img_mouse = imread("./DepthMap/Resources/tsukuba/scene1.row3.col1.jpg", CV_LOAD_IMAGE_COLOR);
...
int main() 
{
	Mat firstImage = imread("./DepthMap/Resources/tsukuba/scene1.row3.col1.jpg", CV_LOAD_IMAGE_GRAYSCALE);
	Mat secondImage = imread("./DepthMap/Resources/tsukuba/scene1.row3.col2.jpg", CV_LOAD_IMAGE_GRAYSCALE);
	...
	unsigned int disparityRange_bp = 25;
	computeDisparity(S, firstImage.rows, firstImage.cols, disparityRange_bp, disparityMap);
	...
	for (int row = 0; row < firstImage.rows; ++row) 
	{
		...
		{
			minCost = MAX_SHORT;
			for (int d = disparityRange_bp - 1; d >= 0; --d) 
			{
				if (minCost > S[row][col][d]) 
				{
					minCost = S[row][col][d];
	...
	char * outFileName = "./DepthMap/Resources/tsukuba/disparity map sgm.jpg";
	...
	Mat color_img = imread("./DepthMap/Resources/tsukuba/scene1.row3.col1.jpg", CV_LOAD_IMAGE_COLOR);
...
// 	}
	...
	string animationDir = "./DepthMap/Resources/maple_leaves/";
