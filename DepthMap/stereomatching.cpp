	...
	for (int d = 0; d < DISPARITY_RANGE; d++) 
	{
		...
		for (int rowIndex = m + 1; rowIndex < DisparityMap.rows - m - 1; rowIndex++) 
		{
			for (int colIndex = m + 1; colIndex < DisparityMap.cols - m - 1; colIndex++) 
			{
				...
				int ssd = (int)getMatElement(integral_ssd, rowIndex + m, colIndex + m) - (int)getMatElement(integral_ssd, rowIndex + m, colIndex - m - 1) - getMatElement(integral_ssd, rowIndex - m - 1, colIndex + m) + getMatElement(integral_ssd, rowIndex - m - 1, colIndex - m - 1);
